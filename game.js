...
function draw(){
  ...
  [zones.entrance,zones.library,zones.forge,zones.hatchery,zones.vault].forEach(z=>ctx.strokeRect(z.x*tile,z.y*tile,z.w*tile,z.h*tile));
  ...
  // forge console
  ctx.fillStyle = '#f2a35c'; ctx.fillRect(23*tile+3,8*tile+3,18,18);
  // hatchery eggs
  if (state.hatcheryUnlocked && !state.dragon) {
    ['#ff7a4d','#8bc6ff','#f1e065'].forEach((c,i)=>{ ctx.fillStyle=c; ctx.beginPath(); ctx.arc((31+i*2)*tile+12,9*tile+12,8,0,Math.PI*2); ctx.fill(); });
  }
  // truth note in vault
  if (!state.truthFound) { ctx.fillStyle='#dbc0ff'; ctx.fillRect(34*tile+6,18*tile+6,12,12); }
  ...
  if (state.dragon) {
    ...
    ctx.beginPath(); ctx.arc((state.player.x-0.6)*tile, (state.player.y+0.2)*tile, 7, 0, Math.PI*2); ctx.fill();
  }
...
function interact(){
  ...
  if (inZone(zones.forge) && Math.hypot(23-p.x,8-p.y) < 2 && state.clues >= 3 && !state.forgeSolved) {
    ...
    panel.querySelectorAll('button').forEach(b => b.onclick = () => {
      ...
      if (ans === 30) {
        state.forgeSolved = true; state.hatcheryUnlocked = true; hidePanel();
        log('Forge powered. Hatchery unlocked.');
      } else {
        state.player.sanity = Math.max(0, state.player.sanity - 12);
        state.monster.x = state.player.x + 2; state.monster.y = state.player.y + 1;
        hidePanel(); log('Wrong. Lights flicker. The Husk is near.');
      }
    });
    return;
  }
...
function showPanel(html){ panel.innerHTML = html; panel.classList.remove('hidden'); state.paused = true; }
function hidePanel(){ panel.classList.add('hidden'); state.paused = false; }
...
function updateMonster(dt){
  ...
  if (m.state === 'Roam') {
    ...
    const vx = state.player.x - m.x, vy = state.player.y - m.y;
